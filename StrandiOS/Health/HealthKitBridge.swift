...
/// macOS importer produces (under the `apple-health` source id), and it writes Baseline-computed metrics
...
final class HealthKitBridge: ObservableObject {
    ...
    /// Baseline's own strap-derived source id, read back when writing into Health.
    private let noopDeviceId: String
    /// Baseline's on-device COMPUTED daily scores (recovery/HRV/RHR/SpO₂/resp) live under the sibling
    ...
    /// then write Baseline's own computed metrics back into Health. Safe to call repeatedly (idempotent
    ...
    func sync(days: Int = 30) async {
        ...
        // HealthKit read of workouts Baseline did NOT author, never any cloud/3rd-party API. (#835)
    ...
    // MARK: - Write back (Baseline → Health)

    /// Write Baseline's strap-derived daily metrics (resting HR, HRV, SpO₂, respiratory rate) into Apple
    ...
    private func writeBack(whoopStore: WhoopStore, days: Int = 14) async throws {
        ...
        // Read Baseline's COMPUTED dailies (deviceId + "-noop"), which is the only place a strap-only
    ...
    /// Excludes Baseline's own write-back samples from reads, so the two-way sync never reads its own
    ...
    /// no cloud or third-party API. Baseline-authored workouts are excluded (the same `notNoopAuthored`
