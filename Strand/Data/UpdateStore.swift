...
final class UpdateStore: ObservableObject {
    ...
    func seedWhatsNewIfNeeded(version: String = AppChangelog.currentVersion,
        ...
        let message = summary ?? String(localized: "Baseline \(version) is here. Tap to read what's new.")
        post(UpdateItem(
            kind: .whatsNew,
            title: title.isEmpty ? String(localized: "What's new in Baseline \(version)") : title,
            message: message
        ))
    }
