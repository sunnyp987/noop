...
/// 4-CSV zip via StrandImport.WhoopCsvExporter. The zip re-imports into Baseline on Mac (Data Sources →
    ...
    static func run(repo: Repository) async -> ExportResult {
        ...
        do {
            ...
            let tmp = try await Task.detached(priority: .utility) {
                ...
                // omitted (read only the two Baseline sources), matching the cycles/sleep exclusion.
            ...
            panel.title = String(localized: "Export Baseline data as CSV")
