            ...
            if !loaded {
                ...
                VStack(alignment: .leading, spacing: NoopMetrics.sectionSpacing) {
                    ...
                    if behaviours.isEmpty {
                        ...
                        NoopCard {
                            Text("Log behaviours above. After a few days of answers, Baseline ranks how each one moves your charge, HRV and rest. Importing a WHOOP export (which includes its journal) backfills history instantly.")
            ...
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Run a clean personal test")
                        ...
                        .foregroundStyle(StrandPalette.textPrimary)
                    Text("Pick one behaviour you log, one outcome, and a short window. Baseline compares the days you log the behaviour against your behaviour-free days before the start.")
                        .font(StrandFont.subhead)
                        .foregroundStyle(StrandPalette.textSecondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    @ViewBuilder private var activityCostSection: some View {
        VStack(alignment: .leading, spacing: NoopMetrics.gap) {
            ...
            if activityCosts.isEmpty {
                NoopCard {
                    Text("Tag a few sessions of the same activity and Baseline will learn its personal recovery cost.")
                        .font(StrandFont.subhead)
                        .foregroundStyle(StrandPalette.textSecondary)
                        .fixedSize(horizontal: false, vertical: true)
                        .frame(maxWidth: .infinity, alignment: .leading)
