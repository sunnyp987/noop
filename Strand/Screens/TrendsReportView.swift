...
struct TrendsReportPage: View {
    ...
    private var header: some View {
        ...
        // contrast carries the edge; the blue Baseline wordmark is the only accent.
        ZStack(alignment: .leading) {
            ...
            VStack(alignment: .leading, spacing: NoopMetrics.space1) {
                HStack(alignment: .firstTextBaseline) {
                    BrandMark(size: 22)
                    Text("Baseline").font(StrandFont.overline).tracking(StrandFont.overlineTracking)
    ...
    private var footer: some View {
        VStack(alignment: .leading, spacing: NoopMetrics.space1) {
            ...
            // are directly measured vs. Baseline's own derived scores. HRV / Resting HR come off the strap;
            // Recovery and Strain are computed on-device and are NOT clinical measures.
            Text("How to read this: HRV, Resting HR, Sleep duration, Respiratory rate and Skin temperature are measured from the strap (skin temp is shown as the deviation from your own baseline). Workouts is the count of activities you logged or that were detected. Recovery, Strain and Stress are Baseline's own on-device scores, not clinical measures: Recovery is a daily readiness composite (HRV, resting HR, sleep and skin-temp trend), Strain is cardiovascular load derived from heart rate, and Stress is a 0-3 autonomic-load index from resting HR and HRV.")
                .font(StrandFont.footnote)
                .foregroundStyle(StrandPalette.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
            Text("Generated by Baseline on \(generatedOn) · all on-device, no account, no cloud.")
                .font(StrandFont.footnote)
                .foregroundStyle(StrandPalette.textTertiary)
...
struct TrendsReportSheet: View {
    ...
    private func export(_ report: RangeReport) {
        ...
        exporting = true
        let name = "Baseline-trends-\(report.start)_to_\(report.end).pdf"
