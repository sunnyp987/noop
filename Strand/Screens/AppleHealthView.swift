        ...
        StrandCard(padding: 20, tint: StrandPalette.metricCyan) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                case .entitlementMissing:
                    ...
                    Text("This install can't connect to Apple Health directly. It was signed with a profile that doesn't include Apple's Health permission, so there's nothing to enable, and Baseline won't appear under Settings › Health.")
                        .font(StrandFont.subhead)
                        .foregroundStyle(StrandPalette.textSecondary)
                        .fixedSize(horizontal: false, vertical: true)
                ...
                case .unknown, .denied:
                    Text("Read your heart rate, HRV, blood oxygen, respiratory rate, sleep, steps and energy straight from Apple Health, and write Baseline's strap-derived metrics back. Everything stays on \(Platform.deviceNounPhrase).")
                        .font(StrandFont.caption)
                        .foregroundStyle(StrandPalette.textTertiary)
                        .fixedSize(horizontal: false, vertical: true)
                    Button {
                    ...
                    if health.auth == .denied {
                        Text("If you don't see the prompt, enable Baseline under Settings › Health › Data Access & Devices.")
                            .font(StrandFont.footnote)
                            .foregroundStyle(StrandPalette.textTertiary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                ...
                case .authorized:
                    if let last = health.lastSync {
                        ...
                        Text("Connected. Reading on launch and when you return to Baseline.")
                            .font(StrandFont.subhead)
                            .foregroundStyle(StrandPalette.textSecondary)
                    }
                    Button {
