...
// conflict-compare sheet that lists EVERY source's value side by side and which one Baseline is using and
...
struct FusedRecordView: View {
    ...
    private var disclaimerNote: some View {
        Text("Baseline picks the best-sourced number and shows you where each came from. It's for wellness and curiosity. It doesn't diagnose or replace medical advice.")
            .font(StrandFont.footnote)
            .foregroundStyle(StrandPalette.textTertiary)
...
/// A small read-only sheet: every source's value for the metric, side by side, with the one Baseline is
/// using marked and its trust reason named. Baseline never adjudicates which is "correct" — it shows the
...
private struct ConflictCompareSheet: View {
    ...
    var body: some View {
        ScreenScaffold(title: LocalizedStringKey(row.label), subtitle: "Your bands report different numbers. Here's every source, and the one Baseline is using.") {
            VStack(alignment: .leading, spacing: NoopMetrics.gap) {
                NoopCard {
                    VStack(spacing: 0) {
                ...
                if let winner = point.contributors.first {
                    HStack(alignment: .top, spacing: 10) {
                        Image(systemName: "info.circle")
                            ...
                            .foregroundStyle(StrandPalette.accent)
                            .accessibilityHidden(true)
                        Text("Baseline shows the \(winner.source.displayName) reading because it \(winner.reason) for this metric: a higher-trust source here, not a verdict that the others are wrong.")
                            .font(StrandFont.subhead)
                            .foregroundStyle(StrandPalette.textSecondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
