...
struct AddDeviceWizard: View {
    ...
    enum DeviceType: Identifiable, Hashable {
        ...
        case oura          // Oura ring (factory-reset-and-adopt: Baseline installs its own key, becomes owner)
        var id: Self { self }
    ...
    /// before the wizard will scan, because adoption installs Baseline's key and the Oura app stops working
    ...
    var body: some View {
        ...
        .alert("Take over this ring?", isPresented: $ouraConfirmAdopt) {
            ...
            Text("Baseline will install its own key on the ring and become its owner. The Oura app will no longer control this ring. This is intended and it cannot be undone from Baseline.")
        }
    ...
    @ViewBuilder private var typeStep: some View {
        VStack(alignment: .leading, spacing: 10) {
            typeRow(.whoop5mg, icon: "applewatch.side.right",
                    title: "WHOOP 5.0 / MG",
                    subtitle: String(localized: "Newer WHOOP band, experimental in Baseline"))
            typeRow(.whoop4, icon: "applewatch.side.right",
                    title: "WHOOP 4.0",
                    subtitle: String(localized: "Baseline's primary, fully-supported band"))
    ...
    private func prepInstructions(_ t: DeviceType) -> [String] {
        ...
        case .whoop4:
            return [
                ...
                String(localized: "Baseline will look for it nearby."),
            ]
        case .whoop5mg:
            return [
        ...
        case .gymEquipment:
            return [
                ...
                String(localized: "Baseline looks for machines that broadcast the standard Bluetooth Fitness Machine service."),
            ]
        case .amazfit:
            return [
                String(localized: "Wake your Amazfit / Zepp band and make sure it isn't connected to the Zepp app right now."),
                String(localized: "Baseline reads live heart rate when the band exposes it. Some bands need a pairing we can't do yet. If so, we'll say so honestly."),
        ...
        case .miBand:
            return [
                ...
                String(localized: "Baseline reads live heart rate on bands that don't require pairing. Newer bands need an auth handshake we can't do yet."),
        ...
        case .oura:
            // The factory-reset-and-adopt checklist, shown only AFTER the irreversible-consent gate. Baseline
            ...
            return [
                String(localized: "Open the official Oura app and remove this ring (Oura calls it \"factory reset\" or \"unpair and reset\"). This wipes the ring's owner so Baseline can take it over."),
    ...
    @ViewBuilder private var ouraConsentGateFace: some View {
        ...
        StrandCard(padding: 16) {
            VStack(alignment: .leading, spacing: 14) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ouraBullet(String(localized: "Your ring talks to Baseline only, fully offline, no Oura account."))
                    ...
                    ouraBullet(String(localized: "Baseline's own Charge, Effort and Rest, computed on your device from published methods."))
                }
                Divider().overlay(StrandPalette.hairline)
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ouraBullet(String(localized: "Oura's own Readiness and Sleep scores. Baseline does not copy them. It computes its own."))
        ...
        Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: ouraConsented ? "checkmark.square.fill" : "square")
                    ...
                    .accessibilityHidden(true)
                Text("I understand this disconnects the ring from Oura and that Baseline cannot undo it for me. To go back to Oura I would factory-reset the ring again and set it up in the Oura app.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textPrimary)
        ...
        .buttonStyle(.plain)
        .accessibilityLabel("I understand this disconnects the ring from Oura and that Baseline cannot undo it for me.")
    ...
    @ViewBuilder private var ouraAdvancedKeyFace: some View {
        ...
        Text("Baseline stores this key only on this device, in the same place it stores your paired bands.")
            .font(StrandFont.footnote)
            .foregroundStyle(StrandPalette.textTertiary)
            .fixedSize(horizontal: false, vertical: true)
    ...
    @ViewBuilder private var ouraConfirmFace: some View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if ouraAdvancedKeyMode {
                ...
                Text("Both Baseline and the Oura app can use a ring you own by key, but only one can hold the Bluetooth link at a time.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            } else {
    ...
    /// The Adopting face: an honest "Installing Baseline's key" progress card shown ONLY while a real key install
    ...
    @ViewBuilder private var ouraAdoptingFace: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Installing Baseline's key and confirming the ring answers only to Baseline. Keep the ring close and do not open the Oura app.")
                .font(StrandFont.subhead)
                .foregroundStyle(StrandPalette.textSecondary)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    @ViewBuilder private var ouraFailedFace: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("We could not take over this ring.")
                .font(StrandFont.headline)
                .foregroundStyle(StrandPalette.textPrimary)
            Text(model.ouraNeedsPairing ?? "The most common cause is the ring was not fully reset in the Oura app, or the Oura app is still running. Reset the ring again, force-quit Oura, then try once more. If it keeps failing, your ring may be a generation Baseline cannot adopt yet. The ring is not bricked: re-pair it in the Oura app to recover it. You can still use file import.")
                .font(StrandFont.subhead)
                .foregroundStyle(StrandPalette.textSecondary)
                .fixedSize(horizontal: false, vertical: true)
            HStack(spacing: 10) {
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("Beta. Read this first.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.statusWarning)
                Text("Local Oura support is new and we cannot test every ring here. It may not connect on your ring, and it can change between updates. Baseline never makes up a number. If something does not work, it will tell you plainly.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.statusWarning)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private var confirmBrand: String {
        ...
        // Oura confirms with the detected generation name + a Beta marker so the user sees what Baseline
    ...
    /// (OuraRingGen.from(model:) recovers it), and the capability set is gen-filtered. Baseline computes its own
    ...
    /// stored here: the live install persists Baseline's freshly-generated key only on an OK `0x25` ack.
    ...
    private var experimentalNote: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "flask")
                .foregroundStyle(StrandPalette.statusWarning)
                .accessibilityHidden(true)
            Text("WHOOP 5.0 / MG support is newer and still experimental in Baseline.")
                .font(StrandFont.footnote)
                .foregroundStyle(StrandPalette.statusWarning)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private var whoopFirstNote: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "info.circle")
                .foregroundStyle(StrandPalette.textTertiary)
                .accessibilityHidden(true)
            Text("WHOOP is Baseline's primary, fully-supported band. Other heart-rate straps stream live heart rate and HRV, but not WHOOP's deeper sleep and recovery data.")
                .font(StrandFont.footnote)
                .foregroundStyle(StrandPalette.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
        }
